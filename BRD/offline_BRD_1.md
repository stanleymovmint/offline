# Table of Contents
1. [Introduction](#Introduction)
   1. [Document Purpose](#Document_Purpose)
   2. [Intended Audience](#Intended_Audience)
   3. [Purpose of the Business Requirements](#Purpose_of_the_Business_Requirements)
   4. [Business goals/objectives to be achieved](#Business_goals/objectives_to_be_achieved)
   5. [Benefits/Rationale](#Benefits/Rationale)
   6. [Stakeholders](#Stakeholders)
   7. [Dependencies on existing systems](#Dependencies_on_existing_systems)
   8. [Assumptions](#Assumptions)
2. [Requirements Scope](#Requirements_Scope)
   1. [In Scope](#In_Scope)
   2. [Out of Scope](#Out_of_Scope)
3. [Functional Requirements](#Functional_Requirements)
   1. [Essential Use Case Diagram](#Essential_Use_Case_Diagram)
   2. [Essential Use Case Specifications](#Essential_Use_Case_Specifications)
   3. [Business Rules](#Business_Rules)
4. [Approval](#Approval)

## Introduction
### Document_Purpose
### Intended_Audience
### Purpose_of_the_Business_Requirements
### Business_goals/objectives_to_be_achieved
### Benefits/Rationale
### Stakeholders
### Dependencies_on_existing_systems
### Assumptions

## Requirements_Scope
### In_Scope
### Out_of_Scope

## Functional_Requirements
### Essential_Use_Case_Diagram
### Essential_Use_Case_Specifications
### Business_Rules

## Approval
